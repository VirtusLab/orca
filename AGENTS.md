...
`orca.agents` + `orca.backend` (LLM SPI, `SessionSupport`, conversation
driver; the SPI is `private[orca]`, so a new harness is added in orca itself),
`orca.subprocess` (subprocess shim), `orca.sweep`
