...
class OrcaLogTest extends munit.FunSuite:
  ...
  test("captures orca DEBUG logs to the trace file; finish is idempotent"):
    ...
    val orcaLog = OrcaLog.start(workDir, AttemptId(Instant.now(), 1))
