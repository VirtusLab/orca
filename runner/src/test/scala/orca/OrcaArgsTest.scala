...
class OrcaArgsTest extends munit.FunSuite:
  ...
  test("--prompt carries a prompt that starts with '-'"):
    assertEquals(
      OrcaArgs.parse(Seq("--prompt", "- add X")).map(_.userPrompt),
      Right("- add X")
    )

  test("a prompt given both positionally and with --prompt is refused"):
    OrcaArgs.parse(Seq("a", "--prompt", "b")) match
      case Left(msg) => assert(msg.contains("--prompt"), msg)
      case Right(r)  => fail(s"expected a refusal, got $r")

  test("toArgv renders a prompt not starting with '-' positionally"):
