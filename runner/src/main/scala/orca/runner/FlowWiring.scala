...
/** The per-run tool/agent override bundle `flow(...)` collects from its named
  ...
  * factory needs it (it pins a `serve` process + drain forks to the run scope,
  * so its factory is applied inside `WiredAgents.build`'s Ox scope); a plain
  * `AgentWiring => Agent` lambda literal auto-adapts to the shape.
  */
