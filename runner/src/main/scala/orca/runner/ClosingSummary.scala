...
private[runner] case class RunChanges(
    base: CommitHash,
    filesChanged: Int,
    countedOn: BranchName
)

/** The block a successful run closes with: where the work ended up, how much of
  * it there is, and the command that shows it.
  *
  * `head` is where HEAD is left, which is not always the run's feature branch:
  * a run that produced nothing has its throwaway branch deleted and ends back
  * where it started.
  */
private[runner] object ClosingSummary:
  ...
  def lines(
    ...
    // Without a worktree the sentence is about HEAD, which is `head` by
    ...
    def where(on: Head): String = worktree match
      case None       => s"done — you are on ${head.describe}"
      case Some(path) => s"done — the work is in $path on ${on.describe}"
    val summary = changes match
      case None => List(where(head))
      case Some(RunChanges(_, 0, _)) =>
        List(where(head), "no files changed")
      case Some(RunChanges(base, files, countedOn)) =>
        ...
        val target =
          ...
          else s"${base.short}..${countedOn.value}"
