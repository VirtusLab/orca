...
private[orca] object SystemPromptComposer:
  ...
  /** Standing rule appended to every write-capable agent turn: orca's runtime
    ...
    * empties the working tree and then turns the runtime's stage commit into a
    * `NothingToCommit` no-op and leaves `git.uncommittedDiff()` empty, so
    * reviewer selection sees no changed files and runs no reviewers. Omitted on
