...
private[orca] object StreamSource:
  ...
  def fromProcess(process: PipedCliProcess): StreamSource =
    new StreamSource:
      ...
      // calls, MCP servers, a build it backgrounded). A root-only kill would orphan that work into the next
      // stage. Work the agent deliberately detached stays out of reach either
      // way.
