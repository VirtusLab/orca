...
/** The channel that connects a flow to its user. `listeners` are registered on
  ...
  * ones. `drive` runs a live interactive turn: it answers the [[ChannelEvent]]s
  * (tool approvals, questions for the user) the [[ObservedTurn]] hands it.
...
trait Interaction:
  ...
  /** Drive a live interactive turn to completion. Returns the final
    * [[AgentResult]] on success, throws [[OrcaInteractiveCancelled]] if the
    * user cancelled mid-turn, or any [[OrcaFlowException]] subtype for other
    * failures.
    */
