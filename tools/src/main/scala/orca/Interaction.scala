...
/** The channel that connects a flow to its user. `listeners` are registered
  ...
  * output, token totals, and errors. `drive` runs a live interactive
  * session: it consumes [[ConversationEvent]]s in order, renders them, and
  * routes user input + tool-approval decisions back via the
  * [[Conversation]] handle. `TerminalInteraction` is the default; a Slack
  * or HTTP implementation can be substituted by passing
  * `interaction = ...` to `flow(...)`.
  */
trait Interaction:
