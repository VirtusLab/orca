...
`create`, `fork`, `edit`, and `continue`'s resume each need a real terminal
and error cleanly if run without one; `run`, `view`, `list`, `config`, and
`rediscover-stack --yes` work fine piped or in CI.
