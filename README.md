...
mints a fresh one per call). Note: opencode sessions are Durable in the
`SessionSupport` sense (a real server-minted `resumeWireId` is committed), but
because orca spawns a fresh `opencode serve --port 0` per run, a *process
restart* always finds that id unreachable and re-seeds — the committed id is
only useful within a single run's server lifetime (live-tested 2026-07-08).
