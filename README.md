...
| `lint(commands, summariser, instructions)` | As above, but summarising into an existing conversation — `Lint.summariserChat(agent)`, which is read-only — instead of a fresh one per call, so a gate run several times within one stage resumes rather than rebuilding the prompt prefix each round. Stop reusing a conversation once it has reported findings: it can repeat them on a later call whose commands no longer show them. `reviewAndFixLoop` does this for you. |
