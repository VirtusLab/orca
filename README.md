...
| `reportingOpenFindings(open)(step)` | Run `step`, then print the "Open review findings" section to the run output, also when `step` throws; nothing when nothing is open. `openPrFromBranch`/`openPrIfGitHub` do this themselves; a flow that writes its own PR body wraps its final PR step in it. |
