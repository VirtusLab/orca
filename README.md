...
structured sibling `resultAs[O].autonomous.run`) primes a fresh session with
...
- **`orca.FlowSession[B]`** — durable, resumable session handle returned by
  ...
  `.run(prompt)` or `.resultAs[O].autonomous.run(input)` on it to drive the
