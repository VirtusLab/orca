...
| `recordOpenedPr(pr)` | Tell the lifecycle a PR was opened, so the run hands the checkout back on the branch it started from. Only for a flow that opens its PR with a bare `gh.createPr` — the two helpers above record it themselves. Call it after the stage that opened the PR returns; it does not compile inside one. |
