...
| `codex` | `mini`, `cheap` (→ mini), `withModel(Model)` | OpenAI Codex coding/reviewing agent. Bare `codex` pins **gpt-5.6-sol**; use `codex.mini` for cheap one-shot calls. The pin is deliberate: `codex exec --json` never names the model on its stream, so an unpinned turn cannot be priced in the cost summary. |
