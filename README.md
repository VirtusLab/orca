...
- **`orca.review.ConfidenceGate(critical, warning, info)`** — the minimum
  confidence a `ReviewIssue` needs to be shown and handed to the fixer, per
  severity; below it, the finding is dropped. `ConfidenceGate.default` is
  `(0.5, 0.6, 0.8)` — an uncertain Critical is still worth a look, an uncertain
  Info is mostly noise.
...
```bash
scala-cli run --workspace "${XDG_CACHE_HOME:-$HOME/.cache}/orca/shell/workspace" --jvm 21 --quiet --dep "org.virtuslab::orca-shell:0.1.2" --main-class orca.shell.Main
```
