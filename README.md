...
| `lint(commands, summariser, instructions)` | As above, but summarising into an existing conversation — `Lint.summariserChat(agent)`, pinned read-only — instead of a fresh one per call, so a gate run several times within one stage resumes rather than rebuilding the prompt prefix each round. A round whose commands are all silent and exit zero still returns an empty result without a turn, so a reused conversation is never asked to judge a clean round. |
