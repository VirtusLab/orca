...
/** Records the `Choice` lists and `preselect` values it was shown, and
  * replays fixed queues of outcomes for `select` and `confirm`.
  * `Wizard.run` only calls `select`; `Wizard.repairMalformed` only calls
  * `confirm`.
  */
...
class WizardTest extends munit.FunSuite:
  ...
  test("first run passes the fallback as preselect for every role"):
    withTempPath: path =>
      ...
      val ui = ScriptedUi(selectScript = List.fill(3)(UiOutcome.Selected(BackendTag.ClaudeCode)))
      assert(Wizard(ui, probe, path).run(reconfigure = false))
      assertEquals(ui.recordedPreselects, List.fill(3)(Some(BackendTag.ClaudeCode)))
  ...
  test("re-configure passes the current harness as preselect, not the fallback"):
    withTempPath: path =>
      os.write(
        path,
        """codingAgent = gemini
      ...
      assertEquals(
        ...
        List(Some(BackendTag.ClaudeCode), Some(BackendTag.Gemini), Some(BackendTag.Gemini))
      )
  ...
  test("re-configure past a malformed file rewrites it from scratch instead of patching junk through"):
    withTempPath: path =>
      ...
      assert(parsed.isRight, s"the rewritten file must parse cleanly, got: $written")
      val agents = parsed.toOption.get.agents
      assertEquals(agents.planning, Some(AgentSpec(BackendTag.ClaudeCode, None)))
      assertEquals(agents.coding, Some(AgentSpec(BackendTag.Codex, None)))
      assertEquals(agents.review, Some(AgentSpec(BackendTag.Gemini, None)))
      assert(!written.contains("not a valid line"), s"the malformed junk must not survive, got: $written")

  test("first run over a pre-existing comments-only file preserves comments via the surgical update"):
    withTempPath: path =>
      os.write(path, "# a hand-written note\n\n")
      val ui = ScriptedUi(
        selectScript = List(
  ...
  test("resolvedSpec keeps the model pin only when the harness is unchanged"):
    ...
    assertEquals(
      Wizard.resolvedSpec(BackendTag.ClaudeCode, Some(BackendTag.ClaudeCode), current),
      AgentSpec(BackendTag.ClaudeCode, Some("sonnet"))
    )
    assertEquals(
      Wizard.resolvedSpec(BackendTag.Gemini, Some(BackendTag.ClaudeCode), current),
  ...
  test("repairMalformed: accepting removes the malformed file and runs the wizard"):
    withTempPath: path =>
      os.write(path, "not a valid line\n")
      val ui = ScriptedUi(
        selectScript = List(
      ...
      assertEquals(agents.coding, Some(AgentSpec(BackendTag.Codex, None)))
      assertEquals(agents.review, Some(AgentSpec(BackendTag.Gemini, None)))
      assert(!written.contains("not a valid line"), s"the malformed file must be replaced, got: $written")

  test("repairMalformed: declining leaves the file untouched and skips the wizard"):
    withTempPath: path =>
      ...
      assertEquals(ui.recordedChoices, Nil, "declining must not run the wizard's role prompts")
