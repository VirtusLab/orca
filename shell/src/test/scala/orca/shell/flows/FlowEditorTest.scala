...
class FlowEditorTest extends munit.FunSuite:
  ...
  test(
    "editArgv wraps the editor in sh -c, passing the path as a single argument"
  ):
  ...
  test(
    "customizeTarget copies a built-in into the project tier via OrcaDir.ensureFlows"
  ):
  ...
  test(
    "customizeTarget refuses to overwrite an existing file at the destination"
  ):
    val workDir = TempDirs.dir()
    val builtIn = TempDirs.dir()
    val flow = builtInFlow(builtIn, "release.sc", "// Release.\nval x = 1")
    os.write(
      workDir / ".orca" / "flows" / "release.sc",
      "// Already customized.\n",
      createFolders = true
    )
