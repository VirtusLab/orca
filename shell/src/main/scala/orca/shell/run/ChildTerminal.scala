...
/** Brackets a foreground child process (a flow run or an editor invocation)
  ...
  * anywhere in the launch chain), so Ctrl-C delivers SIGINT to every process
  * in that group at once: the child dies of its own signal (correct), but
  * the JVM's default SIGINT disposition would kill the shell too without a
  * handler installed here (verified empirically — the shell process exits on
  * Ctrl-C without this bracket). Ignoring the signal for the bracket's
  * duration and restoring the previous handler afterward (the same pattern
  * as sbt's `Signals.withHandler` around `run`/`console`, and git blocking
  * SIGINT/SIGQUIT around the editor/pager) lets Ctrl-C kill only the child;
