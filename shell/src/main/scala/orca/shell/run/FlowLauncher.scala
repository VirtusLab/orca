...
private[shell] object FlowLauncher:
  ...
  /** `ORCA_FLOW_NAME`, read by `runner`'s `flow()`
    * (`orca/flowEntryPoint.scala`) to stamp the run manifest's `flow` field —
    * the flow script's own filename, per the manifest schema
    * (`RunManifest.flow`'s scaladoc examples), unavailable from inside the
