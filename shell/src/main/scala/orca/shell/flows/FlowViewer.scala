...
private[shell] object FlowViewer:
  ...
  /** `source` unchanged when `highlight` is false (piped output stays
    ...
    * the two-argument `build(nanorcUrl, syntaxName)`: jar-verified (jline
    * 3.30.15), no such overload exists — the only classpath-URL overload takes
    * just the nanorc URL, resolving via `Class.getResourceAsStream` on
    * everything after `"classpath:"`.
    */
  def render(source: String, highlight: Boolean): String =
    if !highlight then source
