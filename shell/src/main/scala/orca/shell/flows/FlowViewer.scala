...
object FlowViewer:
  ...
  /** `source` unchanged on a non-tty (piped output stays byte-identical);
    ...
    * [[SyntaxHighlighter]] per call — it carries mutable multi-line-rule
    * state, so reuse across calls would leak between unrelated renders.
    *
    * The single-argument `SyntaxHighlighter.build(String)` is used rather
    * than the two-argument `build(nanorcUrl, syntaxName)` sketched in the
    * task brief: jar-verified (jline 3.30.15), no such overload exists — the
    * only classpath-URL overload takes just the nanorc URL, resolving via
