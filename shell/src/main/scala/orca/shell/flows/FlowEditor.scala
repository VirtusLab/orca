...
object FlowEditor:
  ...
  /** The `sh -c '<editor> "$@"' <editor> <path>` argv (ADR 0021 §6): `editor`
    * is passed as `sh -c`'s `$0` and deliberately shell-interpreted (so `code
    * --wait` splits into command + argument), while `path` travels as `$1` —
    * `"$@"` expands it as a single word regardless of spaces.
    */
  ...
  /** Copies a built-in flow into `tier` (Project or Global) so it can be edited
    * without touching the immutable built-in cache — the copy thereafter
