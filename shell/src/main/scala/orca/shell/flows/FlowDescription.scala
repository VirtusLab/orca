...
object FlowDescription:
  ...
  /** The first `//` comment (not a `//>` directive) in the file's leading block
    ...
    * surrounding whitespace stripped. A comment line that strips to nothing is
    * skipped, not treated as the description. Scanning stops at the first line
    * outside that block (code, or a block-style comment), yielding `None`.
    * `lines` is expected to be a bounded prefix of the file; this function
    * performs no IO and does not limit iteration itself.
    */
