...
private[shell] object CliHelp:
  ...
  val topLevel: String =
    s"""orca shell ${OrcaBuild.current.version} — run, author, and manage Orca flows
       ...
       |
       |Commands:
       |  run <flow> [prompt]        run a flow (prompt from stdin when piped; --branch <name> to name the new branch)
       ...
       |  continue [session|branch]  resume a recorded harness session, by id, name or branch (--list to see them)
