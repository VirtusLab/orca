...
/** Compile-time evidence that no [[InStage]] capability is in scope — i.e. the
  ...
  *
  * Lexical only: a helper declaring `(using FlowContext, FlowControl)` but not
  * `OutsideStage` summons this evidence at its own definition site and
  * compiles, however deep inside a stage its callers sit. Both doors therefore
  * also check at run time (`FlowControl.assertAtFlowBody`); this evidence turns
  * the common case, a call written in a stage body, into a compile error
  * instead of a failure at the end of a run.
  */
@implicitNotFound(
