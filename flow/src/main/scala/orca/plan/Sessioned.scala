...
/** A planning-phase result paired with the (ephemeral) [[Chat]] that produced
  ...
  * durable session; see `agent.session(name, detail, seed)` for that.
  *
  ...
  * `.value` — and mint a durable session via `agent.session(name, detail,
  * seed)`. The chat is in-run only, so a continuation does not survive a
  * crash/resume.
  *
