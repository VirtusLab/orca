...
/** Build a stable, per-run HTML comment marker for use with
  ...
  *
  * `userPrompt` becomes its [[orca.RunKey]], so runs for different prompts
  * produce distinct markers even with the same `purpose`; `purpose` further
  * namespaces the marker within a run (e.g. `"reject"`, `"triage"`). Produces
  * e.g. `<!-- orca:a1b2c3d4e5f6:reject -->`.
  */
def orcaCommentMarker(userPrompt: String, purpose: String): String =
  s"<!-- orca:${RunKey.of(userPrompt).value}:$purpose -->"
