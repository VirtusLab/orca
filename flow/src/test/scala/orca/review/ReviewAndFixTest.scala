...
class ReviewAndFixTest extends munit.FunSuite:
  ...
  test("every finding a reviewer reports reaches the fixer"):
    ...
    val reviewer = new FakeAgent(
      ...
      outputs = List(
        ReviewResult(
          List(finding("a"), finding("b"), finding("c"))
        )
      )
    )
  ...
  test("a finding is shown under the key the fixer is handed for it"):
    ...
    val second = new FakeAgent(
      name = "second",
      outputs = List(ReviewResult(List(finding("c"))))
    )
  ...
  test("a clean exit still names what the fixer declined in an earlier round"):
    ...
    val reviewer = new FakeAgent(
      name = "loud",
      outputs = List(
        ReviewResult(List(finding("driver"), nit)),
        ReviewResult.empty
      )
    )
  ...
  test("the halt exit names what the fixer refused, and its reason"):
    ...
    val reviewer = new FakeAgent(
      ...
      outputs = List(
        ReviewResult(
          List(finding("race in the driver"))
        )
      )
    )
    val coder = new FakeAgent(
      ...
      outputs = List(
        FixOutcome(
          Nil,
          List(OpenFinding(Title("race in the driver"), "the lock covers it"))
        )
      )
    )
    val _ = reviewAndFixLoop(
  ...
  test("the cap exit keeps earlier declines alongside the capped findings"):
    ...
    val reviewer = new FakeAgent(
      name = "loud",
      outputs = List(
        ReviewResult(List(finding("driver"), finding("nit"))),
        ReviewResult(List(finding("stubborn")))
      )
    )
    val coder = new FakeAgent(
      name = "coder",
    ...
    assertEquals(
      ...
      List(
        ...
        OpenFinding(Title("stubborn"), "max iterations (1) reached")
      )
    )
  ...
  test("a paraphrased fixer reply records the finding once, not twice"):
    ...
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(
        FixOutcome(Nil, List(OpenFinding(Title("A real bug"), "not a bug")))
      )
    )
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
    ...
    assertEquals(
      result.findings,
      List(OpenFinding(Title("real bug"), "fixer reported no fixes"))
    )
    val emitted = steps.messages
    assert(
      emitted.contains(
        "Fixer named A real bug, which matched no finding it was handed"
      ),
      emitted.mkString("\n")
    )
  ...
  test(
    ...
    val coder = new FakeAgent(
      ...
      outputs = List(
        ...
        FixOutcome(Nil, List(OpenFinding(Title("nit"), "still deliberate")))
      )
    )
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
    ...
    assertEquals(
      result.findings,
      List(
        OpenFinding(Title("nit"), "still deliberate"),
        OpenFinding(Title("real bug"), "fixer reported no fixes")
      )
    )
  ...
  test("a declined finding the last turn forgot is recorded once, not twice"):
    ...
    assertEquals(
      result.findings,
      List(OpenFinding(Title("nit"), "fixer reported no fixes"))
    )
  ...
  test("a reviewer joining in round three sees round one's declines"):
    ...
    val early = new FakeAgent(
      ...
      outputs = List(
        ReviewResult(
          List(finding("a"), finding("b"))
        ),
        ReviewResult(List(finding("c"))),
        ReviewResult.empty
      )
    )
    ...
    val coder = new FakeAgent(
      ...
      outputs = List(
        FixOutcome(
          List(Title("a")),
          List(OpenFinding(Title("b"), "by design"))
        ),
  ...
  test("runs multiple reviewers and merges their findings"):
    ...
    val reviewerA = new FakeAgent(
      name = "a",
      outputs = List(ReviewResult(List(findingA)))
    )
    val reviewerB = new FakeAgent(
      name = "b",
      outputs = List(ReviewResult(List(findingB)))
    )
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(
        FixOutcome(
          fixed = Nil,
          declined = List(
            OpenFinding(Title("A"), "ok-a"),
            OpenFinding(Title("B"), "ok-b")
          )
        )
      )
    )
  ...
  test("a lint summariser that reports nothing is resumed on later rounds"):
    ...
    val loud = new FakeAgent(
      name = "loud",
      outputs = List.fill(3)(ReviewResult(List(finding("never ends"))))
    )
    ...
    val _ = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
      reviewers = List(asReviewer(loud)),
  ...
  test("a lint summariser that reports findings is not resumed"):
    ...
    val summariser = new FakeAgent(
      name = "summariser",
      outputs = List.fill(2)(ReviewResult(List(finding("lint-found"))))
    )
    val coder = new FakeAgent(
      name = "coder",
  ...
  test("a whole-run diff is re-sampled, so a later round sees the fixes"):
    ...
    val reviewer = new FakeAgent(
      name = "capturing",
      outputs =
        List(ReviewResult(List(finding("needs fixing"))), ReviewResult.empty)
    )
    val coder = new FakeAgent(
      name = "coder",
  ...
  test("a skipped whole-run review still returns the seeded declines"):
    ...
    given FlowControl =
      ReviewLoopFixture.controlWithoutStartingCommit(steps.dispatcher)
    val seeded = OpenFinding(Title("nit"), "the shape is deliberate")
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(new FakeAgent("coder")),
  ...
  test("seeds sharing a title collapse before a skipped review adds its own"):
    ...
    val result = reviewAndFixLoop(
      ...
      priorOpenFindings = OpenFindings(
        List(
          OpenFinding(Title("nit"), "task one declined it"),
          OpenFinding(Title("nit"), "task two declined it too")
        )
      )
    )
    assertEquals(
  ...
  test("the fixer's declines reach the next round's reviewer, its fixes don't"):
    ...
    val coder = new FakeAgent(
      ...
      outputs = List(
        FixOutcome(
          List(Title("real bug")),
          List(OpenFinding(Title("nit"), "the shape is deliberate"))
        )
      )
    )
    val _ = reviewAndFixLoop(
  ...
  test(
    ...
    val reviewerX = new FakeAgent(
      name = "x",
      outputs = List(ReviewResult(List(findingX)))
    )
    ...
    val coder = new FakeAgent(
      name = "coder",
      outputs =
        List(FixOutcome(Nil, List(OpenFinding(Title("only-x"), "accepted"))))
    )
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
    ...
    assertEquals(
      result.findings,
      List(OpenFinding(Title("only-x"), "accepted"))
    )
  ...
  test(
    ...
    val coder = new FakeAgent(
      ...
      outputs = List(
        ...
        FixOutcome(Nil, List(OpenFinding(Title("only-x"), "accepted")))
      )
    )
    given FlowControl =
  ...
  test("the default selection drops a reviewer that reported nothing"):
    ...
    val loud = new FakeAgent(
      name = "loud",
      outputs = List.fill(3)(ReviewResult(List(finding("stubborn"))))
    )
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(
  ...
  test("a lint-only round still leaves the next round with reviewers"):
    ...
    val summariser = new FakeAgent(
      name = "summariser",
      outputs = List.fill(3)(ReviewResult(List(finding("lint-found"))))
    )
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(
  ...
  test("lint runs concurrently with reviewers (deterministic via latch)"):
    ...
    val _ = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(new FakeAgent("coder")),
      reviewers = List(asReviewer(rendezvousReviewer("reviewer"))),
  ...
  test(
    ...
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(
        FixOutcome(Nil, List(OpenFinding(Title("lint-found"), "accepted")))
      )
    )
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
    ...
    assertEquals(
      result.findings,
      List(OpenFinding(Title("lint-found"), "accepted"))
    )
  ...
  test("a selector runs exactly the roster entries it returns"):
    ...
    val coder = new FakeAgent(
      name = "coder",
      outputs = List(FixOutcome(Nil, List(OpenFinding(Title("from-x"), "ok"))))
    )
    val result = reviewAndFixLoop(
      coderSession = ReviewLoopFixture.coderSession(coder),
    ...
    assertEquals(result.findings, List(OpenFinding(Title("from-x"), "ok")))
  ...
  test(
    ...
    def fixPromptWhen(existsResult: Boolean): String =
      ...
      val coder = new SeedProbingCoder(
        ...
        fixOutcome = FixOutcome(Nil, List(OpenFinding(Title("x"), "ok")))
      )
      val reviewer = new FakeAgent(
        name = "r",
        outputs = List(ReviewResult(List(finding("x"))))
      )
      val _ = reviewAndFixLoop(
        coderSession = ReviewLoopFixture.coderSession(coder),
        reviewers = List(asReviewer(reviewer)),
