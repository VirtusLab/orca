...
/** Opaque wrapper for "a JSON subtree, held in its serialized form". Used for
  * message fields whose shape varies too much to worth modelling — tool inputs,
  * arbitrary structured-output payloads. The codec reads and writes the raw
  * bytes straight through without re-parsing.
  *
