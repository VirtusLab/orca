...
private[claude] object StreamEventPayload:
  ...
  private def parseBlockDelta(rawJson: String): StreamEventPayload =
    ...
    delta.`type` match
      case "text_delta" =>
        TextDelta(outer.index, readFromString[TextDeltaWire](outer.delta.value).text)
      case "thinking_delta" =>
        ThinkingDelta(outer.index, readFromString[ThinkingDeltaWire](outer.delta.value).thinking)
  ...
  private def parseBlockStart(rawJson: String): StreamEventPayload =
    ...
    ContentBlockStart(outer.index, ContentBlock.parse(outer.content_block.value))
  ...
  private case class BlockStopWire(index: Int)
      derives ConfiguredJsonValueCodec
